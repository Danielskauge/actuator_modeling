...
def explore_data(data_dir: Path, output_dir: Path):
    ...
    for group_dir in inertia_groups:
        ...
        for csv_file in csv_files:
            ...
            actual_plot_cols_for_hist_in_df = [col for col in PLOT_COLUMNS if col in df.columns] # For histograms
            ...
            if 'Time_ms' in df.columns:
                ...
                if df[time_col_for_plot].nunique() > 1:
                    median_diff = df[time_col_for_plot].diff().median()
                    if median_diff > 0:
                        fs = 1.0 / median_diff
            ...
            if actual_plot_cols_for_hist_in_df:
                ...
                for col in actual_plot_cols_for_hist_in_df:
                    ...
                    sns.histplot(df[col].dropna(), kde=True) # dropna for robustness
            ...
            if time_col_for_plot:
                if accel_cols_present:
                    ...
                    for col in accel_cols_present:
                        sns.lineplot(x=df[time_col_for_plot], y=df[col], label=col)
                    plt.title(f'Accelerometer Data vs. Time\n({group_name} - {csv_file.name})', fontsize=10)
                    plt.xlabel('Time (s)')
                    plt.ylabel('Acceleration') # General unit
                    plt.legend()
                    plot_filename = group_output_dir / f"{csv_file.stem}_ACC_XYZ_timeseries.png"
                    plt.savefig(plot_filename)
                    plt.close()
                ...
                if gyro_cols_present:
                    ...
                    plt.legend()
                    plot_filename = group_output_dir / f"{csv_file.stem}_GYRO_XYZ_timeseries.png"
                    plt.savefig(plot_filename)
                    plt.close()
            ...
            individual_ts_cols_present = [col for col in INDIVIDUAL_TIMESERIES_COLS if col in df.columns]
            ...
            # PSD Plots
            psd_cols_to_plot = [col for col in PSD_TARGET_COLS if col in df.columns]
            ...
            if fs and psd_cols_to_plot:
                ...
                for col in psd_cols_to_plot:
                    ...
                    try:
                        ...
                        plt.psd(df[col].dropna(), NFFT=min(256, len(df[col].dropna())-1 if len(df[col].dropna()) > 1 else 1), Fs=fs) 
                        plt.title(f'PSD of {col}\n({group_name} - {csv_file.name})', fontsize=10)
                        plot_filename = group_output_dir / f"{csv_file.stem}_{col}_psd.png"
                        plt.savefig(plot_filename)
